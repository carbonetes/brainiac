...
usage() (
  this=$1
  cat <<EOF
$this: download go binaries for carbonetes/brainiac
...
check_shasum() {
  if ! command -v shasum >/dev/null; then
    echo "Error: shasum is not installed on your system. Please install shasum first before installing brainiac"
    exit 1
  fi
}
...
install_brainiac() {
  ...
  downloadFolder=$(mktemp -d -t brainiac-XXXXXXXXXX)
