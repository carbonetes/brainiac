# METADATA
# title: "Confirm that CPU limits are configured appropriately."
# description: "Verify that CPU limits are correctly configured for optimal resource utilization in the Kubernetes container."
# scope: package
# related_resources:
...
pass[block] {
	...
	"cpu" in object.keys(resources_block.Attributes.limits)
}
...
passed[result] {
	some block in pass
	result := {
		"message": "CPU limits are configured.",
		"snippet": block,
	}
}
...
failed[result] {
	some block in fail
	result := {
		"message": "CPU limits must be configured.",
		"snippet": block,
	}
}
