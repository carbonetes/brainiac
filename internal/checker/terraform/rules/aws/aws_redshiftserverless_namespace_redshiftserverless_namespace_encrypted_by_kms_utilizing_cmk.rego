...
isvalid(block) if {
	...
	label == "aws_redshiftserverless_namespace"
}
...
passed contains result if {
	some block in pass
	result := {
		"message": "Customer Managed Key (CMK) is used to encrypt the Redshift Serverless namespace.",
		"snippet": block,
	}
}
...
failed contains result if {
	some block in fail
	result := {
		"message": "Customer Managed Key (CMK) must be used to encrypt the Redshift Serverless namespace.",
		"snippet": block,
	}
}
