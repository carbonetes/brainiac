...
isvalid(block) if {
	...
	label == "aws_redshift_snapshot_copy_grant"
}
...
passed contains result if {
	some block in pass
	result := {
		"message": "Customer Managed Key (CMK) is used to encrypt the RedShift snapshot copy.",
		"snippet": block,
	}
}
...
failed contains result if {
	some block in fail
	result := {
		"message": "Customer Managed Key (CMK) must be used to encrypt the RedShift snapshot copy.",
		"snippet": block,
	}
}
