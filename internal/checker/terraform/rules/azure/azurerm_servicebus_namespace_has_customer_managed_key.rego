...
supportedResources := ["azurerm_servicebus_namespace"]
...
passed[result] {
	some block in pass
	result := {"message": "Azure Service Bus utilizes customer-managed keys for data encryption, ensuring data security and control."}
}

failed[result] {
	some block in fail
	result := {"message": "Azure Service Bus must be configured to use customer-managed keys for data encryption to enhance security and maintain control over data protection."}
}